...
export default function Cart({ cart, setCart }) {
  ...
  return (
    <>
      ...
      <h1>Carrito</h1>
            {cart.length > 0 && (
              <button className="btn btn-secondary"  onClick={clearCart} style={ { margin: "25px"} }>Eliminar todo</button>
            )}
            <div className="card-deck mb-3 text-center py-3" >
              {cart.map((product, idx) => (
                ...
                <div className="product"  style={ { marginBottom: "25px"} }>
                  ...
                  <button className="btn btn-danger" onClick={() => removeFromCart(product)} style={ { marginTop: "25px"} }>
                    Eliminar
                  </button>
                    </div>
                  </div>
